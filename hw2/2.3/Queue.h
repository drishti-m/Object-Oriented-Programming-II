...
void Queue<T>::resize(int size)
{
    ...
    //sets the max size required after resizing
    if (last < size)
        limit = last;
    else
        limit = size;
    ...
    ptr = newptr; //point to new ptr
    this->size = size; //update new size
    last = limit; //update new last
...
int Queue<T>::getSize()
{
    return size; //gets max size it can store
}
